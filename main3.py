...
# --- YENİ: İpucu metni artık .env dosyasından okunuyor ---
VOCAB_PROMPT = os.getenv("VOCAB_PROMPT", "")
...
def transcribe_audio(file_path, root_folder):
    ...
    Terminoloji için ipucu metnini .env dosyasından alır.
    """
    ...
    try:
        ...
        # --- GÜNCELLEME: initial_prompt .env'den gelen VOCAB_PROMPT'u kullanıyor ---
...
def main():
    ...
    if VOCAB_PROMPT:
        log_with_timestamp("Bilgi: .env dosyasından özel kelime dağarcığı (VOCAB_PROMPT) yüklendi.")
    else:
        log_with_timestamp("Uyarı: Özel kelime dağarcığı (VOCAB_PROMPT) .env dosyasında bulunamadı veya boş.")
